...
public class FixedSalesReportController implements Initializable
{
    private Stage stage;
    private Scene scene;
    private Parent root;
    ...
    public void loadYearlyReport()
    {
        if (yearDropbox.getValue() != null)
        {
            ...
            try
            {
                String hatchEggs = " SELECT sum(Hatching_Eggs * Hatching_Eggs_Price) " +
                        " FROM  NewSales   WHERE YEAR(Date_Sold) = " + yearDropbox.getValue() + ";";

                String pullets = " SELECT sum(Pullets * Pullet_Price) " +
                        " FROM  NewSales   WHERE YEAR(Date_Sold) =  " + yearDropbox.getValue() + ";";
                
                String cockerels = " SELECT sum(Cockerels * Cockerel_Price) " +
                        " FROM  NewSales   WHERE YEAR(Date_Sold) = " + yearDropbox.getValue() + ";";
                
                String hens = " SELECT sum(Hens * Hen_Price) " +
                        " FROM  NewSales   WHERE YEAR(Date_Sold) =  " + yearDropbox.getValue() + ";";
                
                String roosters = " SELECT sum(Roosters * Rooster_Price) " +
                        " FROM  NewSales   WHERE YEAR(Date_Sold) =  " + yearDropbox.getValue() + ";";

                String chicks = " SELECT sum(Chicks * Chick_Price) " +
                        " FROM  NewSales   WHERE YEAR(Date_Sold) =  " + yearDropbox.getValue() + ";";

                String egg12 = " SELECT sum(Eating_Eggs12 * Eating_Eggs12_Price) " +
                        " FROM  NewSales   WHERE YEAR(Date_Sold) =  " + yearDropbox.getValue() + ";";

                String egg6 = " SELECT sum(Eating_Eggs6 * Eating_Eggs6_Price) " +
                        " FROM  NewSales   WHERE YEAR(Date_Sold) =  " + yearDropbox.getValue() + ";";
                ...
                PreparedStatement ps10 = connection.prepareStatement(hens);
                PreparedStatement ps11 = connection.prepareStatement(roosters);
                PreparedStatement ps12 = connection.prepareStatement(cockerels);
                ...
                rs10 = connection.createStatement().executeQuery(hens);

                ResultSet rs11 = ps11.executeQuery();
                rs11 = connection.createStatement().executeQuery(roosters);
                
                ResultSet rs12 = ps12.executeQuery();
                rs12 = connection.createStatement().executeQuery(cockerels);
                ...
                if (rs.next())
                {
                    ...
                    if (rs10.next())
                        henLabel.setText("$ " +f.format(rs10.getDouble(1)));

                    if (rs11.next())
                        roosterLabel.setText("$ " +f.format(rs11.getDouble(1)));

                    if (rs12.next())
                        cockerelLabel.setText("$ " +f.format(rs12.getDouble(1)));

                    profitLabel.setText("$ " +f.format(rs.getDouble(1) 
                            + rs1.getDouble(1) 
                            + rs2.getDouble(1) 
                            + rs3.getDouble(1)
                            + rs10.getDouble(1)
        ...
        if (monthDropbox.getValue()!=null && yearDropbox.getValue()!=null)
        {
            ...
            if (yearDropbox.getValue() != null)
            {
                ...
                try
                {
                    ...
                    String hatchEggs = " SELECT sum(Hatching_Eggs * Hatching_Eggs_Price) " +
                            " FROM  NewSales   WHERE YEAR(Date_Sold) = " + yearDropbox.getValue() + " AND MONTH(Date_Sold) = " + month + ";";
    ...
    public void loadDateEarningsReport()
    {
        ...
        if (fromDate != null && toDate!=null)
        {
            ...
            try
            {
                ...
                String hatchEggs = " SELECT sum(Hatching_Eggs * Hatching_Eggs_Price) " +
                        " FROM  NewSales   WHERE Date_Sold BETWEEN '"+fromDate.getValue().toString()+"' AND '"+toDate.getValue().toString()+"';";
